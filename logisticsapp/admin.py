...
@admin.register(City)
class City(ImportExportModelAdmin):
    list_display =['id','city_name']

@admin.register(CustomUser)
class CustomUser(ImportExportModelAdmin):
    ...
    list_display =['id','user_role_name','city','vehicle','first_name','last_name','mobile_number','company_name','email','login_status','on_going_ride','user_online_status','alternate_number','zip_code','address','adhar_card','reset_otp','profile_image_path','base64','adhar_card_front_side_img','adhar_card_front_side_img_path','adhar_card_back_side_img','adhar_card_back_side_img_path','pan_card','pan_image_path','pan_card_base64','whatsup_number', 'user_status', 'user_active_status']
...
class Driver(ImportExportModelAdmin):
...
class Customised_message(ImportExportModelAdmin):
    list_display = ['id','driver_id','message_type']


@admin.register(DriverDocumentStatus)
class DriverDocumentStatus(ImportExportModelAdmin):
    list_display =['id','status_name']


@admin.register(Remarks)
class Remarks(ImportExportModelAdmin):
    list_display =['id','driver_id','document_status','text']

@admin.register(DriverDocumentExpiryvalidity)
class DriverDocumentExpiryvalidity(ImportExportModelAdmin):
...
class Schedulehour(ImportExportModelAdmin):
    list_dispaly = ['id','time']

@admin.register(RideType)
class RideType(ImportExportModelAdmin):
    list_dispaly = ['id','ride_type']

@admin.register(SelectedRideType)
class SelectedRideType(ImportExportModelAdmin):
    list_dispaly = ['id','trip_type']
