    ...
    def get(self, request, order_id, user_id):
        ...
        for i in list(final_order_detail):
            ...
            if average_rating['rating__avg'] is None:
                ...
                i['driver__driver_id__rating'] = average_rating['rating__avg']
            ...
            if data['vehicle_number'] is not None:
                ...
                if Driver.objects.filter(vehicle__vehicletypes_id=data['vehicle_type'], is_online=True):
                    ...
                    for i in driver_obj_location:
                        ...
                        if BookingDetail.objects.filter((Q(driver_id=i['user_id']) & (Q(status_id=1) | Q(status_id=2) | Q(status_id=6) | Q(status_id=8) | Q(status_id=10))) & Q(is_scheduled=False)) and s.date() == datetime.datetime.today().date():
...
class AllScheduledOrder(APIView):
    ...
    def post(self, request):
        ...
        if BookingDetail.objects.filter(Q(driver_id=driver_id) & (Q(status_id=1) | Q(status_id=2) | Q(status_id=6) | Q(status_id=8) | Q(status_id=9) | Q(status_id=10))).exists():
...
# booking_obj = BookingDetail.objects.filter(driver_id=348).values('order_id')
